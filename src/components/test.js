json_string = '{\n' +
    '  "cod": "200",\n' +
    '  "message": 0,\n' +
    '  "cnt": 40,\n' +
    '  "list": [\n' +
    '    {\n' +
    '      "dt": 1686171600,\n' +
    '      "main": {\n' +
    '        "temp": 22.73,\n' +
    '        "feels_like": 23.23,\n' +
    '        "temp_min": 22.73,\n' +
    '        "temp_max": 24.2,\n' +
    '        "pressure": 1010,\n' +
    '        "sea_level": 1010,\n' +
    '        "grnd_level": 1004,\n' +
    '        "humidity": 83,\n' +
    '        "temp_kf": -1.47\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 0\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.24,\n' +
    '        "deg": 334,\n' +
    '        "gust": 3.01\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-07 21:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686182400,\n' +
    '      "main": {\n' +
    '        "temp": 22.83,\n' +
    '        "feels_like": 23.15,\n' +
    '        "temp_min": 22.83,\n' +
    '        "temp_max": 23.04,\n' +
    '        "pressure": 1010,\n' +
    '        "sea_level": 1010,\n' +
    '        "grnd_level": 1003,\n' +
    '        "humidity": 76,\n' +
    '        "temp_kf": -0.21\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 801,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "few clouds",\n' +
    '          "icon": "02n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 13\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 0.74,\n' +
    '        "deg": 358,\n' +
    '        "gust": 1.69\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 00:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686193200,\n' +
    '      "main": {\n' +
    '        "temp": 22.83,\n' +
    '        "feels_like": 22.97,\n' +
    '        "temp_min": 22.83,\n' +
    '        "temp_max": 22.88,\n' +
    '        "pressure": 1011,\n' +
    '        "sea_level": 1011,\n' +
    '        "grnd_level": 1004,\n' +
    '        "humidity": 69,\n' +
    '        "temp_kf": -0.05\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 67\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.61,\n' +
    '        "deg": 31,\n' +
    '        "gust": 1.71\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 03:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686204000,\n' +
    '      "main": {\n' +
    '        "temp": 26.3,\n' +
    '        "feels_like": 26.3,\n' +
    '        "temp_min": 26.3,\n' +
    '        "temp_max": 26.3,\n' +
    '        "pressure": 1011,\n' +
    '        "sea_level": 1011,\n' +
    '        "grnd_level": 1004,\n' +
    '        "humidity": 51,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 100\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.81,\n' +
    '        "deg": 138,\n' +
    '        "gust": 1.61\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 06:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686214800,\n' +
    '      "main": {\n' +
    '        "temp": 28.48,\n' +
    '        "feels_like": 28.52,\n' +
    '        "temp_min": 28.48,\n' +
    '        "temp_max": 28.48,\n' +
    '        "pressure": 1011,\n' +
    '        "sea_level": 1011,\n' +
    '        "grnd_level": 1004,\n' +
    '        "humidity": 45,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 97\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 3.31,\n' +
    '        "deg": 146,\n' +
    '        "gust": 2.58\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 09:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686225600,\n' +
    '      "main": {\n' +
    '        "temp": 27.17,\n' +
    '        "feels_like": 27.52,\n' +
    '        "temp_min": 27.17,\n' +
    '        "temp_max": 27.17,\n' +
    '        "pressure": 1010,\n' +
    '        "sea_level": 1010,\n' +
    '        "grnd_level": 1003,\n' +
    '        "humidity": 49,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 99\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 0.46,\n' +
    '        "deg": 214,\n' +
    '        "gust": 1.24\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 12:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686236400,\n' +
    '      "main": {\n' +
    '        "temp": 27.9,\n' +
    '        "feels_like": 28.09,\n' +
    '        "temp_min": 27.9,\n' +
    '        "temp_max": 27.9,\n' +
    '        "pressure": 1009,\n' +
    '        "sea_level": 1009,\n' +
    '        "grnd_level": 1002,\n' +
    '        "humidity": 47,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 96\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 3.11,\n' +
    '        "deg": 220,\n' +
    '        "gust": 2.92\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 15:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686247200,\n' +
    '      "main": {\n' +
    '        "temp": 24.73,\n' +
    '        "feels_like": 24.75,\n' +
    '        "temp_min": 24.73,\n' +
    '        "temp_max": 24.73,\n' +
    '        "pressure": 1009,\n' +
    '        "sea_level": 1009,\n' +
    '        "grnd_level": 1002,\n' +
    '        "humidity": 57,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 98\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.54,\n' +
    '        "deg": 293,\n' +
    '        "gust": 3.02\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 18:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686258000,\n' +
    '      "main": {\n' +
    '        "temp": 23.53,\n' +
    '        "feels_like": 23.51,\n' +
    '        "temp_min": 23.53,\n' +
    '        "temp_max": 23.53,\n' +
    '        "pressure": 1009,\n' +
    '        "sea_level": 1009,\n' +
    '        "grnd_level": 1002,\n' +
    '        "humidity": 60,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 58\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.36,\n' +
    '        "deg": 308,\n' +
    '        "gust": 2.51\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-08 21:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686268800,\n' +
    '      "main": {\n' +
    '        "temp": 23.16,\n' +
    '        "feels_like": 23.02,\n' +
    '        "temp_min": 23.16,\n' +
    '        "temp_max": 23.16,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 57,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 76\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.34,\n' +
    '        "deg": 331,\n' +
    '        "gust": 1.74\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 00:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686279600,\n' +
    '      "main": {\n' +
    '        "temp": 23.31,\n' +
    '        "feels_like": 23.13,\n' +
    '        "temp_min": 23.31,\n' +
    '        "temp_max": 23.31,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 55,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 99\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.9,\n' +
    '        "deg": 60,\n' +
    '        "gust": 1.98\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 03:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686290400,\n' +
    '      "main": {\n' +
    '        "temp": 26.59,\n' +
    '        "feels_like": 26.59,\n' +
    '        "temp_min": 26.59,\n' +
    '        "temp_max": 26.59,\n' +
    '        "pressure": 1009,\n' +
    '        "sea_level": 1009,\n' +
    '        "grnd_level": 1002,\n' +
    '        "humidity": 49,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 70\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.23,\n' +
    '        "deg": 139,\n' +
    '        "gust": 1.94\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 06:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686301200,\n' +
    '      "main": {\n' +
    '        "temp": 29.36,\n' +
    '        "feels_like": 29.18,\n' +
    '        "temp_min": 29.36,\n' +
    '        "temp_max": 29.36,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 42,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 10\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.93,\n' +
    '        "deg": 151,\n' +
    '        "gust": 2.89\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0.15,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 09:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686312000,\n' +
    '      "main": {\n' +
    '        "temp": 28.3,\n' +
    '        "feels_like": 28.6,\n' +
    '        "temp_min": 28.3,\n' +
    '        "temp_max": 28.3,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 48,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 8\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.67,\n' +
    '        "deg": 187,\n' +
    '        "gust": 1.76\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0.11,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 12:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686322800,\n' +
    '      "main": {\n' +
    '        "temp": 28.8,\n' +
    '        "feels_like": 28.87,\n' +
    '        "temp_min": 28.8,\n' +
    '        "temp_max": 28.8,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 45,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 1\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.8,\n' +
    '        "deg": 236,\n' +
    '        "gust": 2.86\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0.04,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 15:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686333600,\n' +
    '      "main": {\n' +
    '        "temp": 26.04,\n' +
    '        "feels_like": 26.04,\n' +
    '        "temp_min": 26.04,\n' +
    '        "temp_max": 26.04,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 50,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 1\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.65,\n' +
    '        "deg": 309,\n' +
    '        "gust": 3.6\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0.08,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 18:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686344400,\n' +
    '      "main": {\n' +
    '        "temp": 24.6,\n' +
    '        "feels_like": 24.66,\n' +
    '        "temp_min": 24.6,\n' +
    '        "temp_max": 24.6,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 59,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 1\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.69,\n' +
    '        "deg": 28,\n' +
    '        "gust": 2.09\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-09 21:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686355200,\n' +
    '      "main": {\n' +
    '        "temp": 23.48,\n' +
    '        "feels_like": 23.45,\n' +
    '        "temp_min": 23.48,\n' +
    '        "temp_max": 23.48,\n' +
    '        "pressure": 1005,\n' +
    '        "sea_level": 1005,\n' +
    '        "grnd_level": 998,\n' +
    '        "humidity": 60,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 2\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.29,\n' +
    '        "deg": 232,\n' +
    '        "gust": 2.18\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 00:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686366000,\n' +
    '      "main": {\n' +
    '        "temp": 22.76,\n' +
    '        "feels_like": 22.74,\n' +
    '        "temp_min": 22.76,\n' +
    '        "temp_max": 22.76,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 63,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 4\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.46,\n' +
    '        "deg": 261,\n' +
    '        "gust": 2.66\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 03:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686376800,\n' +
    '      "main": {\n' +
    '        "temp": 27.01,\n' +
    '        "feels_like": 27.31,\n' +
    '        "temp_min": 27.01,\n' +
    '        "temp_max": 27.01,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 48,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 3\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.65,\n' +
    '        "deg": 223,\n' +
    '        "gust": 2.78\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 06:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686387600,\n' +
    '      "main": {\n' +
    '        "temp": 28.86,\n' +
    '        "feels_like": 29.04,\n' +
    '        "temp_min": 28.86,\n' +
    '        "temp_max": 28.86,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 46,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 9\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 4.39,\n' +
    '        "deg": 221,\n' +
    '        "gust": 4.8\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 09:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686398400,\n' +
    '      "main": {\n' +
    '        "temp": 29.82,\n' +
    '        "feels_like": 29.83,\n' +
    '        "temp_min": 29.82,\n' +
    '        "temp_max": 29.82,\n' +
    '        "pressure": 1005,\n' +
    '        "sea_level": 1005,\n' +
    '        "grnd_level": 998,\n' +
    '        "humidity": 43,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 5\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 7.97,\n' +
    '        "deg": 242,\n' +
    '        "gust": 7.64\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 12:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686409200,\n' +
    '      "main": {\n' +
    '        "temp": 28.12,\n' +
    '        "feels_like": 28.32,\n' +
    '        "temp_min": 28.12,\n' +
    '        "temp_max": 28.12,\n' +
    '        "pressure": 1005,\n' +
    '        "sea_level": 1005,\n' +
    '        "grnd_level": 998,\n' +
    '        "humidity": 47,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 0\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 6.98,\n' +
    '        "deg": 256,\n' +
    '        "gust": 7.95\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 15:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686420000,\n' +
    '      "main": {\n' +
    '        "temp": 23.96,\n' +
    '        "feels_like": 24.08,\n' +
    '        "temp_min": 23.96,\n' +
    '        "temp_max": 23.96,\n' +
    '        "pressure": 1005,\n' +
    '        "sea_level": 1005,\n' +
    '        "grnd_level": 998,\n' +
    '        "humidity": 64,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 0\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 4.06,\n' +
    '        "deg": 269,\n' +
    '        "gust": 4.85\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 18:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686430800,\n' +
    '      "main": {\n' +
    '        "temp": 22.4,\n' +
    '        "feels_like": 22.6,\n' +
    '        "temp_min": 22.4,\n' +
    '        "temp_max": 22.4,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 73,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 0\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.89,\n' +
    '        "deg": 288,\n' +
    '        "gust": 3.25\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-10 21:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686441600,\n' +
    '      "main": {\n' +
    '        "temp": 21.56,\n' +
    '        "feels_like": 21.76,\n' +
    '        "temp_min": 21.56,\n' +
    '        "temp_max": 21.56,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 76,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 6\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.28,\n' +
    '        "deg": 286,\n' +
    '        "gust": 2.6\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 00:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686452400,\n' +
    '      "main": {\n' +
    '        "temp": 21.13,\n' +
    '        "feels_like": 21.47,\n' +
    '        "temp_min": 21.13,\n' +
    '        "temp_max": 21.13,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 83,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 802,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "scattered clouds",\n' +
    '          "icon": "03d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 50\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.28,\n' +
    '        "deg": 299,\n' +
    '        "gust": 1.86\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 03:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686463200,\n' +
    '      "main": {\n' +
    '        "temp": 24.53,\n' +
    '        "feels_like": 24.81,\n' +
    '        "temp_min": 24.53,\n' +
    '        "temp_max": 24.53,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 68,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 802,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "scattered clouds",\n' +
    '          "icon": "03d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 27\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.8,\n' +
    '        "deg": 198,\n' +
    '        "gust": 2.46\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 06:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686474000,\n' +
    '      "main": {\n' +
    '        "temp": 26.69,\n' +
    '        "feels_like": 27.7,\n' +
    '        "temp_min": 26.69,\n' +
    '        "temp_max": 26.69,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 60,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 0\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 5.04,\n' +
    '        "deg": 211,\n' +
    '        "gust": 4.14\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 09:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686484800,\n' +
    '      "main": {\n' +
    '        "temp": 27.36,\n' +
    '        "feels_like": 27.91,\n' +
    '        "temp_min": 27.36,\n' +
    '        "temp_max": 27.36,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 52,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 801,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "few clouds",\n' +
    '          "icon": "02d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 19\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 6.48,\n' +
    '        "deg": 232,\n' +
    '        "gust": 6.37\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 12:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686495600,\n' +
    '      "main": {\n' +
    '        "temp": 26.19,\n' +
    '        "feels_like": 26.19,\n' +
    '        "temp_min": 26.19,\n' +
    '        "temp_max": 26.19,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 50,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 802,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "scattered clouds",\n' +
    '          "icon": "03d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 43\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 5.81,\n' +
    '        "deg": 244,\n' +
    '        "gust": 6.01\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 15:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686506400,\n' +
    '      "main": {\n' +
    '        "temp": 22.51,\n' +
    '        "feels_like": 22.57,\n' +
    '        "temp_min": 22.51,\n' +
    '        "temp_max": 22.51,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 67,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 802,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "scattered clouds",\n' +
    '          "icon": "03n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 42\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 3.51,\n' +
    '        "deg": 271,\n' +
    '        "gust": 4.21\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 18:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686517200,\n' +
    '      "main": {\n' +
    '        "temp": 21.62,\n' +
    '        "feels_like": 21.61,\n' +
    '        "temp_min": 21.62,\n' +
    '        "temp_max": 21.62,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 68,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 74\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 1.86,\n' +
    '        "deg": 285,\n' +
    '        "gust": 2.79\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-11 21:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686528000,\n' +
    '      "main": {\n' +
    '        "temp": 20.62,\n' +
    '        "feels_like": 20.62,\n' +
    '        "temp_min": 20.62,\n' +
    '        "temp_max": 20.62,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 72,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 68\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 0.89,\n' +
    '        "deg": 288,\n' +
    '        "gust": 2.34\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 00:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686538800,\n' +
    '      "main": {\n' +
    '        "temp": 20.47,\n' +
    '        "feels_like": 20.53,\n' +
    '        "temp_min": 20.47,\n' +
    '        "temp_max": 20.47,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 75,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 801,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "few clouds",\n' +
    '          "icon": "02d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 24\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 0.82,\n' +
    '        "deg": 0,\n' +
    '        "gust": 2.27\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 03:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686549600,\n' +
    '      "main": {\n' +
    '        "temp": 23.77,\n' +
    '        "feels_like": 23.8,\n' +
    '        "temp_min": 23.77,\n' +
    '        "temp_max": 23.77,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 61,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 803,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "broken clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 62\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 2.23,\n' +
    '        "deg": 224,\n' +
    '        "gust": 2.46\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 06:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686560400,\n' +
    '      "main": {\n' +
    '        "temp": 26.54,\n' +
    '        "feels_like": 26.54,\n' +
    '        "temp_min": 26.54,\n' +
    '        "temp_max": 26.54,\n' +
    '        "pressure": 1008,\n' +
    '        "sea_level": 1008,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 50,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 100\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 5.43,\n' +
    '        "deg": 226,\n' +
    '        "gust": 5.07\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 09:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686571200,\n' +
    '      "main": {\n' +
    '        "temp": 27,\n' +
    '        "feels_like": 27.48,\n' +
    '        "temp_min": 27,\n' +
    '        "temp_max": 27,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1000,\n' +
    '        "humidity": 51,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 804,\n' +
    '          "main": "Clouds",\n' +
    '          "description": "overcast clouds",\n' +
    '          "icon": "04d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 86\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 6.87,\n' +
    '        "deg": 230,\n' +
    '        "gust": 6.22\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 12:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686582000,\n' +
    '      "main": {\n' +
    '        "temp": 25.48,\n' +
    '        "feels_like": 25.57,\n' +
    '        "temp_min": 25.48,\n' +
    '        "temp_max": 25.48,\n' +
    '        "pressure": 1006,\n' +
    '        "sea_level": 1006,\n' +
    '        "grnd_level": 999,\n' +
    '        "humidity": 57,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01d"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 2\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 4.97,\n' +
    '        "deg": 238,\n' +
    '        "gust": 5.12\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "d"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 15:00:00"\n' +
    '    },\n' +
    '    {\n' +
    '      "dt": 1686592800,\n' +
    '      "main": {\n' +
    '        "temp": 22.12,\n' +
    '        "feels_like": 22.32,\n' +
    '        "temp_min": 22.12,\n' +
    '        "temp_max": 22.12,\n' +
    '        "pressure": 1007,\n' +
    '        "sea_level": 1007,\n' +
    '        "grnd_level": 1001,\n' +
    '        "humidity": 74,\n' +
    '        "temp_kf": 0\n' +
    '      },\n' +
    '      "weather": [\n' +
    '        {\n' +
    '          "id": 800,\n' +
    '          "main": "Clear",\n' +
    '          "description": "clear sky",\n' +
    '          "icon": "01n"\n' +
    '        }\n' +
    '      ],\n' +
    '      "clouds": {\n' +
    '        "all": 2\n' +
    '      },\n' +
    '      "wind": {\n' +
    '        "speed": 3.52,\n' +
    '        "deg": 259,\n' +
    '        "gust": 4.06\n' +
    '      },\n' +
    '      "visibility": 10000,\n' +
    '      "pop": 0,\n' +
    '      "sys": {\n' +
    '        "pod": "n"\n' +
    '      },\n' +
    '      "dt_txt": "2023-06-12 18:00:00"\n' +
    '    }\n' +
    '  ],\n' +
    '  "city": {\n' +
    '    "id": 146384,\n' +
    '    "name": "Limassol",\n' +
    '    "coord": {\n' +
    '      "lat": 34.698,\n' +
    '      "lon": 33.0465\n' +
    '    },\n' +
    '    "country": "CY",\n' +
    '    "population": 154000,\n' +
    '    "timezone": 10800,\n' +
    '    "sunrise": 1686105285,\n' +
    '    "sunset": 1686157122\n' +
    '  }\n' +
    '}'


const data = JSON.parse(json_string);
const periods = data.list;

let count = 0;
periods.forEach(per => {
    count += 1;
    const dt_xtx = per.dt_txt; // дата в текстовом формате
    const date = new Date(per.dt * 1000);
    const d = date.getUTCDate();
    const hours = date.getUTCHours();
    const minutes = date.getUTCMinutes();
    const formatted_date = date.toLocaleDateString('ru');

    const t_min = per.main.temp_min;
    const t_max = per.main.temp_max;
    const wind = per.wind.speed;
    const wind_max = per.wind.gust;
    const weather = per.weather[0].main;
    const clouds = per.clouds.all;
    console.log(`${count}: ${formatted_date} ${hours} t=${t_min}-${t_max} ${weather} clouds ${clouds}`);
});
console.log(`we have ${periods.length}`)

function interpolate(left, right, index) {
    let delta = right - left;
    return (left + delta / 3.0 * index);
}

function get_24_hours(array) {
    console.log(`array length = ${array.length}`);
    const weather_24_hours = [];
    weather_24_hours.length = 25;
    count
    let ind = 0;
    array.forEach(elem => {
            const date = new Date(elem.dt * 1000);
            const dayname = date.toLocaleString("en", {weekday: "short"});
            const hours = date.getUTCHours();
            const t = elem.main.temp_max;//Math.round(elem.main.temp_max);
            const clouds = elem.clouds.all;
            const wind = elem.wind.speed;
            const hour_weather = {
                hour: hours,
                temp: t,
                clouds: clouds,
                wind: wind
            }
            weather_24_hours[ind] = hour_weather;
            ind += 3;
        }
    );
    // теперь заполним пустые ячейки - их по 2 между непустыми элементами
    for (let i = 0; i < 25; i++) {
        // пропускаем элементы, четные 3-м
        const rest = i % 3;
        if (rest === 0) {
            continue;
        }
        // индексы предыдущего и последующего непустых элементов
        let left = i - 1;
        let right = i + 2;
        // если текущий индекс четный, значит поправим индексы
        if (rest == 2) {
            left--;
            right--;
        }
        // создаем элементы, вычисляем его поля на основе значений соседних непустых элементов
        const prev = weather_24_hours[left];
        const next = weather_24_hours[right];
        let hour = prev.hour + rest;
        if (hour == 24) {
            hour = 0;
        }
        const hour_weather = {
            hour: hour,
            temp: Number(interpolate(prev.temp, next.temp, rest).toFixed(1)),
            clouds: Number(interpolate(prev.clouds, next.clouds, rest).toFixed(0)),
            wind: Number(interpolate(prev.wind, next.wind, rest).toFixed(2))
        }
        weather_24_hours[i] = hour_weather;
    }

    return (weather_24_hours.slice(0, -1));
}


const weather_hourly = get_24_hours(periods.slice(0, 9));
console.log(weather_hourly);


function get_days(array) {
    console.log(`array length = ${array.length}`);
    const weather_days = [];
    let temp_max = -100;
    let temp_min = +100;
    let wind_max = 0;
    let wind_min = 0;
    let current_day_name = '';
    const first_date = new Date(array[0].dt * 1000)
    let prev_day_name = first_date.toLocaleString("en", {weekday: "short"});
    let day_count = 0;
    let clouds_sum = 0;
    let dayname = '';
    array.forEach(elem => {
            const date = new Date(elem.dt * 1000);
            dayname = date.toLocaleString("en", {weekday: "short"});
            day_count++;
            // новый день недели
            if (dayname !== prev_day_name) {
                // создаем объект
                const day_weather = {
                    day: prev_day_name,
                    temp_max: Number(temp_max.toFixed(0)),
                    temp_min: Number(temp_min.toFixed(0)),
                    wind_max: Number(wind_max.toFixed(0)),
                    wind_min: Number(wind_min.toFixed(0)),
                    clouds: Number((clouds_sum / day_count).toFixed(0)),
                }
                weather_days.push(day_weather);
                // сбросим в исходной состояние
                temp_max = -100;
                temp_min = +100;
                wind_max = 0;
                wind_min = 0;
                clouds_sum = 0;
                day_count = 0;
                // запомним последний день
                prev_day_name = dayname;
            }
            temp_max = Math.max(temp_max, elem.main.temp_max);
            temp_min = Math.min(temp_min, elem.main.temp_min);
            clouds_sum += elem.clouds.all;
            wind_max = Math.max(wind_max, elem.wind.speed);
            wind_min = Math.min(wind_min, elem.wind.speed);
        }
    );
    // допишем хвост
    const day_weather = {
        day: prev_day_name,
        temp_max: Number(temp_max.toFixed(0)),
        temp_min: Number(temp_min.toFixed(0)),
        wind_max: Number(wind_max.toFixed(0)),
        wind_min: Number(wind_min.toFixed(0)),
        clouds: Number((clouds_sum / day_count).toFixed(0)),
    }
    weather_days.push(day_weather);
    // массив дней погоды
    return (weather_days);
}


const weather_daily = get_days(periods);
console.log(weather_daily);